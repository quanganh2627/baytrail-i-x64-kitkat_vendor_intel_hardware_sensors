...
bool AudioClassifierSensor::selftest() {
    ...
    ALOGE("Pedometer sensor self test failed!");
    return false;
}
...
inline void AudioClassifierSensor::connectToPSH() {
    if (isConnectToPSH()) {
        ALOGE("psh session is already opened");
        return;
    }
    ...
    if (mAudioHandle == PSH_SESSION_NOT_OPENED) {
        ALOGE("psh_open_session failed. retry once");
...
inline int AudioClassifierSensor::stopStreamForPSH() {
    ALOGD("before psh_stop_streaming 0");
    int ret = 0;
    if (isConnectToPSH()) {
        ALOGD("before psh_stop_streaming");
        if (methods.psh_stop_streaming(mAudioHandle) != ERROR_NONE) {
            ALOGE("psh_stop_streaming failed.");
            usleep(SLEEP_ON_FAIL_USEC);
            if (methods.psh_stop_streaming(mAudioHandle) != ERROR_NONE) {
                ALOGE("psh_stop_streaming failed.");
                ret = -1;
        ...
        ALOGD("after psh_stop_streaming");
        if (ret == 0)
            ALOGI("PSH streaming stopped.");
    }
    ALOGD("after psh_stop_streaming 0");
    return ret;
}
...
void AudioClassifierSensor::AudioHAL::audioHalActivate() {
    ...
    if (device != NULL) {
        if (device->activate_aware_session((hw_device_t*) device,
                aware_classifier) == 0)
            ALOGD("activated aware successful...\n");
        else
            ALOGE("activated aware failed\n");
    }
...
void AudioClassifierSensor::AudioHAL::audioHalDeActivate() {
    ...
    ALOGD("deactivating aware session...");
    if (device != NULL) {
        if (device->deactivate_aware_session((hw_device_t*) device) == 0)
            ALOGD("deactivated aware session successful... \n");
        else
            ALOGE("deactivated aware session failed \n");
...
int AudioClassifierSensor::activate(int32_t handle, int en) {
    ALOGI("enable");
    if (!isResultPipeSetup()) {
        ALOGE("Invalid status while enable");
        return -1;
    }
    if (mEnabled == en) {
...
int AudioClassifierSensor::setDelay(int32_t handle, int64_t ns) {
    ...
    if (ns != SENSOR_DELAY_TYPE_AUDIO_CLASSIFICATION_INSTANT * 1000
        ...
        ALOGE("Delay not supported");
        return -1;
    }
    if (mEnabled == 0) {
        ALOGE("setDelay while not enabled");
        return -1;
    }
    if (mCurrentDelay == ns) {
        ALOGI("Setting the same delay, do nothing");
        return 0;
    }
...
void AudioClassifierSensor::stopWorker() {
    if (mWorkerThread == THREAD_NOT_STARTED) {
        ALOGI("Stop worker thread while thread is not running");
        return;
    }
    // when thread is active, the wake up pipes should be valid
    if (!isThreadWakeupPipeSetup()) {
        ALOGE("isThreadWakeupPipeSetup = false");
    }
...
int AudioClassifierSensor::getData(std::queue<sensors_event_t> &eventQue) {
    ...
    while (size > 0) {
        ...
        ALOGI("Event value is %d", *p_audio_data);
        eventQue.push(event);
        numEventReceived++;
        size = size - unit_size;
        p = p + unit_size;
    ...
    ALOGI("read %d events", numEventReceived);
...
int AudioClassifierSensor::workerThread(void *data) {
    ...
    if (src->startStreamForPSH() == -1) {
        ALOGE("psh_start_stream failed.");
        goto err_handle_start;
    }
    if (src->propertySetForPSH(nTimeDelay) == -1) {
        ALOGE("psh_set_property failed.");
        goto err_handle_set;
    }
    ...
    while (poll(polls, 2, -1) > 0) {
        ...
        if (size != sizeof(audioData)) {
            ALOGE("Read End Unexpectedly, Size: %d", size);
            break;
        }
        ALOGI("Update classifier %d dB %d",
