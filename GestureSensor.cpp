          ...
          mEnabled(0)
{
        ...
        if (mLibraryHandle != NULL) {
                ...
                if (mGestureInit != NULL && mGestureProcessSingleData != NULL && mGestureClose != NULL) {
                        ...
                        ALOGI("Got all required functions");
                } else {
                        ALOGE("Can't get all required functions!!");
                }
        } else {
                ALOGE("Can't find libgesture!!");
        }
}

GestureSensor::~GestureSensor()
{
        ALOGI("~GestureSensor %d\n", mEnabled);
        Stop_accel();
        Stop_gyro();
        Stop_proximity();
...
int GestureSensor::activate(int32_t handle, int en)
{
        ...
        ALOGI("GestureSensor - %s - enable=%d", __FUNCTION__, en);
        if (!mHasGestureLibrary) {
                ALOGE("Gesture library error!");
                return -1;
        }
        ...
        if (en == 1) {
                ...
                if (!acclThreadStarted || !proximityThreadStarted || !gyroThreadStarted) {
                        ALOGE("Failed to start gesture or proximity thread");
                        Stop_accel();
                        Stop_gyro();
                        Stop_proximity();
                        return -1;
...
{
        ALOGI("setDelay - %s - %lld", __FUNCTION__, ns);
        return 0;
}

int GestureSensor::getPollfd()
{
        ALOGI("Fd is %d, %d", mResultPipe[0], mResultPipe[1]);
...
bool GestureSensor::selftest()
{
        if (mResultPipe[0] != PIPE_NOT_OPENED &&
            ...
            mHasGestureLibrary) {
                ...
                ALOGI("Gesture sensor self test failed");
                return false;
        }
}
int GestureSensor::getData(queue<sensors_event_t> & eventQue)
{
        ...
        ALOGI("GestureSensor - getData");

        if (mResultPipe[0] == PIPE_NOT_OPENED) {
                ALOGI("invalid status ");
                return 0;
        }
        size = (512 / unit_size) * unit_size;

        ALOGI("Try to read %d", size);
        size = read(mResultPipe[0], buf, size);
        ALOGI("Actually read %d", size);

        char *p = buf;
        while (size > 0) {
                p_gesture_data = (int *)(p);
                event.data[0] = (*p_gesture_data);
                ALOGI("Event value is %d", *p_gesture_data);
        ...
        ALOGI("GestureSensor - read %d events", numEventReceived);
...
{
        ALOGD("context thread start: wait for accel data");
        ...
        while ((ret = (select(max_fd +1, &read_fds, NULL, NULL, NULL)) >= 0)) {
                ...
                if (FD_ISSET(tSrc->mWakeFdsAccel[0], &read_fds)) {
                        ALOGD("context thread end - gesture");
                        break;
                }
                ...
                while (size > 0) {
                        ...
                        //ALOGD("%hd %hd %hd %hd %hd %hd", data[0], data[1], data[2], data[3], data[4], data[5]);
                        ...
                        if (gesture != NULL) {
                                ...
                                if ((f1 && f2) || ((!f1) && (!f2))) {
                                        ...
                                        if (gestureResult != INVALID_GESTURE_RESULT) {
                                                ALOGI("write into result pipe start");
                                                write(tSrc->mResultPipe[1], (char*)(&gestureResult),
                                                      sizeof(gestureResult));
                                                ALOGI("write into result pipe end");
                                        }
                                }
                        }
                        delete [] gesture;
...
{
        ALOGD("context thread start: wait for proximity data");
        ...
        while ((ret = (select(max_fd +1, &read_fds, NULL, NULL, NULL)) >= 0)) {
                ...
                if (FD_ISSET(termSrc->mWakeFdsProximity[0], &read_fds)) {
                        ALOGD("context thread end - proximity");
                        break;
                }
                ...
                while (size > 0) {
                        ALOGD("-- proximity: %d", p_ps_phy_data->near);
...
{
        ALOGD("context thread start: wait for gyro data");
        GestureSensor* termSrc = static_cast<GestureSensor*>(source);
...
bool GestureSensor::Start_accel()
{
        ...
        mInitGesture = true;
        if (r == true) {
                ALOGD("init psh sensor hub - accel");
        ...
        Stop_accel();
        ALOGE("psh & algorithm start return false - gesture");
        return false;
}
...
{
        ALOGD("init psh sensor hub - proximity");
        ...
        Stop_proximity();
        ALOGE("psh start return false - proximity");
        return false;
}
...
{
        if (mHandleProximity != PSH_SESSION_NOT_OPENED) {
                ALOGD("stop sensor hub - proximity");
...
{
        ALOGD("init psh sensor hub - gyro");
        ...
        Stop_gyro();
        ALOGE("psh start return false - gyro");
        return false;
}
